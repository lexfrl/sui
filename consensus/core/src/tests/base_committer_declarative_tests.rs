...
/// Here we setup a situation where we have an undecided leader (D)
/// and it's possible what we indirectly skip it or there is a chance that B is equivoking
/// so we encounter B votes for D in the wave 1 making it a commit
#[tokio::test]
async fn test_equivoking_direct_commit() {
    telemetry_subscribers::init_for_testing();
    let context = Arc::new(Context::new_for_test(4).0);
    let dag_state = Arc::new(RwLock::new(DagState::new(
        context.clone(),
